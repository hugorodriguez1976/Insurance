...
namespace Insurance.WebClient.Helper
{
    public class PremiumModelHelper
    {
        ...
        public List<SelectListItem> PopulatePremiumVM()
        {
            ...
            var modelTaskOccupations = Task.Run(() => proxy.GetRequest<List<Occupation>>("api/occupation/getall")).Result;
        ...
        {
            ClientProxy proxy = new ClientProxy(webapiurl);
            double premium = Task.Run(() => proxy.GetRequest<double>(string.Format("api/occupation/calculatepremium?occupationid={0}&amount={1}&age={2}", OccupationId, Amount, Age))).Result;
            return premium;
        }
